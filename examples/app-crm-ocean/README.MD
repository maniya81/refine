...
<div align="center">Ocean Technolab — CRM frontend built for ease of use and automated workflows.<br>The UI is implemented with React + TypeScript and Ant Design. It uses the Refine library as a technical integration (data/UI primitives) rather than being a Refine project itself.
<br />
<br />
...
This frontend was created as a product UI (and reference) using React, Ant Design and GraphQL. It integrates the Refine library for common admin patterns (data hooks, providers), but the product is Ocean Technolab's CRM — not a Refine example project. Feel free to reuse components and patterns; refer to the `src/providers` folder for data and auth wiring.

👉 You can find a more comprehensive CRM example [here](https://example.crm.refine.dev/).

## Project (Ocean Technolab)

This workspace contains the frontend for Ocean Technolab's CRM product. It is a Refine + Ant Design TypeScript frontend focused on simplicity for customers and automated data flows. Key product points:

- Auto-sync from Meta (Facebook) lead sources into the CRM (leads ingestion).
- Frontend-only repository: UI, routes, GraphQL queries and Refine data-provider customization live here.
- Backend services (Python API) are maintained in a separate repository; the frontend talks to that API. Set `VITE_API_URL` in your environment to point at the backend (defaults to `http://localhost:8000/api`).

If you need to update backend behavior (endpoints, pagination, or special-case GETs), coordinate with the Python API repo and update `src/providers/data/index.ts` to keep the client and server in sync.
...
Clone the repository and install dependencies, then start the dev server:

```powershell
git clone <repo-url>
cd OceanCRM_Web_App_Refine
npm install
npm run dev
```

Open `http://localhost:5173` in your browser. Set `VITE_API_URL` to your backend if it runs on a different host.

## Project structure

Quick overview of the main folders and files to help you navigate the codebase:

- `src/`
  - `routes/` — page-level route components (lists, create/edit/view pages).
  - `components/` — shared UI components (layout, avatar, tags, accordion, etc.).
  - `providers/` — central wiring: `data` (API/dataProvider), `auth`, `access-control`.
  - `services/` — domain helpers and higher-level API wrappers.
  - `graphql/` — GraphQL queries/mutations and generated types (optional).
  - `utilities/` — small helpers (organization scoping, date helpers, formatting).
  - `config/` — runtime config or resource definitions.

- Top-level files
  - `package.json` — scripts and dependencies.
  - `vite.config.ts` / `tsconfig.json` — build & TypeScript config.
  - `.env.*` — environment templates for local/dev/production.

Use this as a quick map when adding pages or tracing data flows: UI -> `src/routes/*` -> `src/providers/data/index.ts` -> backend (`VITE_API_URL`).

## Developer Guide (short)

Quick orientation for new frontend developers:

- Repo purpose: this is the Ocean Technolab CRM frontend (React + TypeScript + Ant Design). The project uses the Refine library as a UI/data helper rather than being a Refine tutorial — treat Refine as a library.

- Key folders:
  - `src/routes/` — page components and route composition. Add new pages here.
  - `src/components/` — shared UI components (avatars, tags, layout pieces).
  - `src/providers/` — central wiring (auth, `dataProvider`, access control). See `src/providers/data/index.ts` for API shapes.
  - `src/services/` — domain helpers that call the API.
  - `src/graphql/` — GraphQL operations and generated types (optional).
  - `src/utilities/` — small helpers (organization scoping, date helpers).

- Adding a new UI page:
  1. Create a folder under `src/routes/<name>/` and add `list.tsx`, `create.tsx`, `edit.tsx`, `view.tsx` as needed.
  2. Use Refine hooks (`useList`, `useForm`, `useModalForm`) and provide `resource` strings that map to backend endpoints.
  3. If the page uses GraphQL operations, add them to `src/graphql/` and run `npm run codegen` to update types.
  4. Mirror exports from other routes and verify top-level routing in `src/App.tsx`.

- API and data-provider:
  - API base is defined in `src/providers/data/index.ts` as `API_BASE_URL` / `API_URL` (fallback `http://localhost:8000/api`).
  - `axiosInstance` adds `x-org-id` and token headers; toggle token vs cookie auth with `VITE_AUTH_METHOD`.
  - `getList` expects `page` and `page_size` and uses `q` for search; arrays are passed as repeated query params (e.g., `assigned_user_ids`).
  - Note: `getOne('lead')` currently performs a large-list fetch then finds the item client-side — prefer adding a backend GET endpoint and updating `getOne`.

- Env & common commands:
  - `VITE_API_URL` — backend base URL (default `http://localhost:8000/api`).
  - `VITE_AUTH_METHOD` — `token` or cookie-based auth.
  - Install: `npm install`
  - Dev: `npm run dev`
  - Build: `npm run build`
  - GraphQL/types: `npm run codegen`
  - Format / lint: `npm run format` / `npm run lint` (uses Biome)

  Formatting details: we use Biome for formatting and linting. Configuration is in `biome.json` at the repo root. The npm scripts run Biome via `npx`, e.g.:
  ...
  You can run `npm run format` and `npm run lint` which wrap these commands.

Additional environment notes:
- **Token vs Cookie auth:**
  - `token` mode: frontend stores `access_token`, `refresh_token`, `access_csrf`, `refresh_csrf` in `localStorage`. The data provider/`axiosInstance` adds `x-access-token` and `x-refresh-token` headers when present.
  - `cookie` mode: backend must set auth and CSRF cookies; frontend uses `axiosInstance` with `withCredentials` enabled and checks for `csrf_access_token` cookie during auth checks.
- **Organization keys (localStorage):** `org_id`, `org_list`, `current_org` — used by `src/utilities/organization.ts` and sent as `x-org-id` header on requests.
- **Trailing-slash / FastAPI:** the client strips trailing slashes for requests to avoid `307` redirects from FastAPI. Prefer stable backend routes (no 307 redirects) or update `src/providers/data/index.ts` accordingly.

## Example .env

Create a local env file for development (e.g. `.env.local` or `.env.development`) with the following example values:

```bash
# Backend API URL (adjust to your backend host)
VITE_API_URL=http://localhost:8000/api

# Auth method: set to `token` to use token-based auth (localStorage)
# or `cookie` to use cookie-based auth (backend sets auth cookies)
VITE_AUTH_METHOD=token

# Enable Facebook SDK / lead import UI (feature flag)
VITE_SHOW_FACEBOOK_LEAD_IMPORT=false

# Optional Facebook SDK config (recommended when using FB import UI)
# Set these in your environment rather than hard-coding them in source code
VITE_FACEBOOK_APP_ID=YOUR_FACEBOOK_APP_ID
VITE_FACEBOOK_API_VERSION=v24.0
```

Notes:
- `token` mode stores `access_token` and `refresh_token` in `localStorage` (keys: `access_token`, `refresh_token`). The frontend then sends `x-access-token` / `x-refresh-token` headers.
- `cookie` mode relies on the backend to set authentication cookies and on `axiosInstance.withCredentials` (no tokens in localStorage).
- After changing env vars, restart the dev server.

- PR checklist when touching API behavior:
  - Keep `dataProvider` shapes consistent or update `src/providers/data/index.ts`.
  - Document backend contract changes and coordinate with the Python API repo.
  - Run `npm run format` and `npm run lint` before opening PRs.

CI & deployment notes:
- GitHub Actions: `.github/workflows/ci.yaml` runs Biome checks on Node 22.
- Deploy: `.github/workflows/deploy.yaml` triggers backend workflows via the `gh` CLI and expects a repository secret (see workflow for `RENDER_GH_PAT`). Document required secrets in a safe place (organization secrets or internal docs).
- Hosting: `netlify.toml` contains a SPA redirect; if deploying to Netlify set `VITE_API_URL` in Netlify build environment.

GraphQL codegen:
- `npm run codegen` runs the configured GraphQL Codegen. If you do not have a committed `graphql.config.ts` or `codegen.yml`, create one and ensure generated types under `src/graphql/` are committed or produced in CI before builds.

Facebook SDK note:
- The Facebook UI currently loads the SDK and uses an appId/version in code. Prefer using env variables (`VITE_FACEBOOK_APP_ID`, `VITE_FACEBOOK_API_VERSION`) and fallbacks when initializing the SDK in `src/routes/facebook/index.tsx`.

Repository housekeeping suggestions:
- Add a top-level `.env.example` (we included a sample above) and commit it.
- Add `CONTRIBUTING.md` and a `LICENSE` file if appropriate for your project.
- Tests: this repo currently has no unit tests; consider adding `vitest` and a `test` script to run in CI.
